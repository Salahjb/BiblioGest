...
namespace BiblioGest.Models
{
    public class BiblioDbContext : DbContext
    {
        ...
        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
        {
            if (!optionsBuilder.IsConfigured)
            {
                // IMPORTANT: Make sure the password field is included if required
                var connectionString = "server=localhost;port=3306;database=biblioguest;user=root;password=";

                // Enable sensitive data logging to help debug connection issues
                optionsBuilder
